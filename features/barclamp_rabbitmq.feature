...
Feature: Test the AMQP Messaging service
  ...
  Scenario: RabbitMQ successful deployment
    ...
    And the node with rabbitmq-server role has been detected successfully
    Then I can check the status of rabbitmq-server using command line
    When I add a new user and view the user in the users list
    And I change the user's role to an administrator
